...
expect_equal_to_reference(ProductTestRaw  %>% sngl(s2b)(2,3)  %>% report, "sngl1.rds")

expect_equal_to_reference(ProductTestRaw  %>% check_subset(cell %in% 2) %>% sngl(s2b)(2,3)  %>% report, "sngl2.rds") 
...
expect_equal_to_reference(ProductTestRaw  %>% check_if(a3 %in% 1:4) %>% mult(a4_1:a4_6)(valid_a4,exclusive=99)  %>% report,
                          "mult4.rds")
expect_equal_to_reference(ProductTestRaw %>% check_subset(cell %in% 2) %>% check_if(a3 %in% 1:4) %>% mult(a4_1:a4_6)(valid_a4,exclusive=99)  %>% report, 
                          "mult5.rds")

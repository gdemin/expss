...
context("from, to notation simple vector")

expect_identical(if_val(1:5, from = 1, to = -1), c(-1, 2, 3, 4, 5))
expect_identical(if_val(1:5, from = 1:2, to =c(-1, NA)), c(-1, NA, 3, 4, 5))
expect_identical(if_val(1:5, from = ">2", to=99), c(1, 2, 99, 99, 99))
expect_identical(if_val(1:5, from = c(">2","."), to = c(99,0)), c(0, 0, 99, 99, 99))
...
expect_identical(if_val(1:5, from = c("<2", "<3", "<4","."), to =c(10,  11, 12, NA)), c(10, 11, 12, NA, NA))
...
expect_identical(if_val(x, from = ">2", to =list(y)), c(1, 8, 1, 9, NA))
expect_identical(if_val(x, from = ">2", to = list(y)), c(1, 8, 1, 9, NA))
expect_identical(if_val(x, from = list(">2", "<=2"), to = list(y,z)), c(4, 8, 4, 9, NA))
expect_identical(if_val(x, from = list(">2", "<=2","."), to = list(y,z,99)), c(4, 8, 4, 9, 99))

expect_identical(if_val(x, from = list(z>4), to = list(y)), c(1, 3, 1, 9, 9))

context("from, to notation dplyr")
library(dplyr)
library(magrittr)

...
expect_identical(category(t(t(c(0,1,0,1,0,1))),compress=TRUE),structure(c(NA, 1L, NA, 1L, NA, 1L), .Dim = c(6L, 1L)))
expect_identical(category(t(c(0,1,0,1,0,1)),compress=TRUE), structure(c(2L, 4L, 6L), .Dim = c(1L, 3L)))
expect_identical(category(c(0,1,0,1,0,1),compress=TRUE),structure(c(NA, 1L, NA, 1L, NA, 1L), .Dim = c(6L, 1L)))

set.seed(123)
dichotomy_matrix = matrix(sample(0:1,40,replace = TRUE,prob=c(.6,.4)),nrow=10)
colnames(dichotomy_matrix) = c("Milk","Sugar","Tea","Coffee")

expect_equal_to_reference(category(dichotomy_matrix, prefix = "products_",compress=TRUE), "rds/category4.rds")

if(isTRUE(getOption("covr"))){ 
  ...
  expect_equal_to_reference(as_hux(mtcars_table) ,
                            "rds/as_hux14.rds",  update = FALSE)
  
  mtcars_table = cro_cpct(list(mtcars$vs), 
                          list(mtcars$vs %nest% mtcars$am, "#Total")) 
  
  expect_equal_to_reference(as_hux(mtcars_table),
                            "rds/as_hux14_2.rds",  update = FALSE)
  ...
  expect_equal_to_reference(as_hux(res), "rds/as_hux15.rds",  update = FALSE)
  ...
  expect_equal_to_reference(as_hux(res), "rds/as_hux16.rds",  update = FALSE)
  
  # single row header
  res = mtcars %>% calc(cro(am, list(unvr(vs))))
  expect_equal_to_reference(as_hux(res), "rds/as_hux17.rds",  update = FALSE)
  
  # single row header
  res = mtcars %>% calc(cro(list(unvr(am)), list(unvr(vs))))
  expect_equal_to_reference(as_hux(res), "rds/as_hux18.rds",  update = FALSE)
  
  # temp = function(x) as_hux:::print.as_hux(x, useViewer = TRUE)
  
  res = fre(mtcars$cyl)
  expect_equal_to_reference(as_hux(res), "rds/as_hux19.rds",  update = FALSE)

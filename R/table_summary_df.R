...
table_pearson = function(summary_vars,
    ...
    cor_fun = function(x, weight = NULL){
        res = w_pearson(x, weight = weight)
        dtfrm(res_num = seq_len(nrow(res)), label = rownames(res), cor = res[,1])
    }
    table_summary_df(summary_vars = summary_vars,
                     col_vars = col_vars,
                     fun = cor_fun,
                     weight = weight,
                     subset = subset,
...
table_spearman = function(summary_vars,
    ...
    cor_fun = function(x, weight = NULL){
        res = w_spearman(x, weight = weight)
        dtfrm(res_num = seq_len(nrow(res)), label = rownames(res), cor = res[,1])
    }
    table_summary_df(summary_vars = summary_vars,
                     col_vars = col_vars,

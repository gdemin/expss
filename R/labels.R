...
#' 
#' @aliases val_lab<- set_val_lab unvl add_val_lab add_val_lab<-
...
#' @export
set_val_lab.data.frame = function(x,value){
    for (i in seq_along(x)) val_lab(x[[i]]) = value
    x
}

#' @export
set_val_lab.list = function(x,value){
    for (i in seq_along(x)) val_lab(x[[i]]) = value
    x
}

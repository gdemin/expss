...
#' @param frequencies Logical. Should we calculate frequencies for each variable? This calculation can
#' take significant amount of time for large datasets.
#' @param max_levels Numeric. Maximum levels for using in frequency calculations. Levels above 
#' this value will convert to 'Other values'.
#' @return data.frame with following columns: Name, Class, Length, NotNA, Label, ValueLabels, Frequency.
#' @details Resulting data.frame of this function mainly intended to be saved as csv to keep before eyes
#' in RStudio viewer or spreadsheet software as reference about working dataset. 
#' @examples
#' data(mtcars)
...
#' @export
info=function(x, frequencies = TRUE, max_levels= 10){
    UseMethod("info")
}

#' @export
info.default=function(x, frequencies = TRUE, max_levels= 10){
...
#' @export
info.data.frame=function(x, frequencies = TRUE, max_levels= 10){
    info.list(x,frequencies,max_levels)
}

info.matrix=function(x, frequencies = TRUE, max_levels= 10){
    info.list(as.data.frame(x,stringsAsFactors = FALSE),frequencies,max_levels)
}

#' @export
info.list=function(x, frequencies = TRUE, max_levels= 10){

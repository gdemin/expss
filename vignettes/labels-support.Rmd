...
Value labels are similar to variable labels, but value labels are descriptions of the values a variable can take. Labeling values means you don’t have to remember if 1=Extremely poor and 7=Excellent or vice-versa. You can easily get dataset description and variables summary with `info` function.

The usual way to connect numeric data to labels in R is in factor variables. However, factors miss important features which the value labels provide. Factors only allow for integers to be mapped to a text label, these integers have to be a count starting at 1 and every value need to be labelled. Also, you can’t calculate means or other numeric statistics on factors. 

With labels you can manipulate short variable names and codes when you analyze your data but in the resulting tables and graphs you will see human-readable text. 
...
There are special methods for subsetting and concatenating labelled variables. These methods preserve labels during common operations. You don't need to restore labels on subsetted or sorted data.frame. 
